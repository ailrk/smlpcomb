...
Token
Parser::next_tok() {
    return tok = lexer.get_token();
}

int
Parser::token_precedence() {
    if (tok.istok()) {
        return -1;
    }
...
std::unique_ptr<Expr>
Parser::numberexpr() {
    auto result = std::make_unique<NumerExpr>(lexer.n_val);
    next_tok();
...
Parser::identifierexpr() {
    ...
    if (tok.value() != ')') {
        while (true) {
            ...
            if (tok.value() != ',') {
                return error<Expr>("Expected ')' or ',' in argument list");
            }
            next_tok(); // eat ','
        }
    }
...
Parser::primary() {
    ...
    case '(':
        return parenexpr();
    default:
        return error<Expr>("unknown token for primary expression");
    }
}

// binoprhs       ::= (biop primary)*
std::unique_ptr<Expr>
Parser::binoprhs(int exprprec, std::unique_ptr<Expr> lhs) {
    while (true) {
...
Parser::prototype() {
    if (tok.value() != TokenType::TokenIdentifier) {
        return error<Prototype>("Expected function name");
    }
    ...
    if (tok.value() != '(') {
        return error<Prototype>("Expected ( in prototype");
    }
